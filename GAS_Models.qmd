...
-   Utilizar essa abordagem ao invés de outras utilizadas em outros modelos faz com que os modelos baseados em *score* sejam naturalmente mais robustoz a *outliers*.
...
- Aqui, indica a direção de máximo crescimento da verossimilhança dos retornos (TA CERTO ISSO??)

# Comparações

## Score x Momento

-   def: modelos baseados no momento são aqueles onde a variável que guia a função atualização possui esperança igual ao parâmetro de interesse (Ex: Garch)
...
$$
y_t = \sigma_t\epsilon_t \\
\sigma_t^2 = \omega + \alpha s_t + \beta\sigma^2_t \\
...
## Score x Volatilidade Estocástica
